...
function showSecondSelect2() {
    var firstSelect = document.getElementById("choose-pay2").value;
    var secondSelectContainer = document.getElementById("select2Container2");
    var secondSelect = document.getElementById("select22");
    ...
    if (firstSelect === "choose-pay-mobile-banking2") {
        options = ["Vietcombank", "Techcombank", "VPBank", "BIDV", "MBBank", "Agribank"];
    } else if (firstSelect === "choose-pay-atm2") {
        options = ["Visa", "Mastercard", "American Express", "JCB", "Discover"];
    } else if (firstSelect === "choose-pay-cash2") {
        options = ["Không có"];
    }
    ...
    //   <!-- thông tin trong table  -->
        function saveInfo2() {
                var name = document.getElementById("custom2").value;
                var phone = document.getElementById("phone2").value;
                var address = document.getElementById("address2").value;
                var car = document.getElementById("car2").value;
                var paymentMethod = document.getElementById("choose-pay2").value;
                var bank = document.getElementById("select22").value;
                var price = document.getElementById("money2").value;

                if (!name || !phone || !address || !car || !paymentMethod || !bank) {
                    alertWarning2("Vui lòng điền đầy đủ thông tin!");
                } else {
                    if (paymentMethod === "choose-pay-cash2") {
                       paymentMethod = "Tiền mặt";
                    } else if (paymentMethod === "choose-pay-mobile-banking2") {
                       paymentMethod = "Chuyển khoản qua Ngân Hàng";
                    } else if (paymentMethod === "choose-pay-atm2") {
                ...
                if(typeof(Storage) !== "undefined")
                    {
                        try {
                            ...
                            // Lưu mảng đã cập nhật vào localStorage
                             ...
                             alertSuccess2("Đặt hàng thành công");
       ...
       function alertSuccess2(message) {
         var alertBox = $('<div class="alert alert-success alert-dismissible fade show" role="alert">' + 
                         ...
                         '</div>');
         $('#alerts2').append(alertBox);
         setTimeout(function() {
          alertBox.alert('close');
          }, 1000); 
       }

       function alertWarning2(message) {
