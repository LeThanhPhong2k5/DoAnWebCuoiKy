...
function showSecondSelect3() {
    var firstSelect = document.getElementById("choose-pay3").value;
    var secondSelectContainer = document.getElementById("select2Container3");
    var secondSelect = document.getElementById("select23");
    ...
    if (firstSelect === "choose-pay-mobile-banking3") {
        options = ["Vietcombank", "Techcombank", "VPBank", "BIDV", "MBBank", "Agribank"];
    } else if (firstSelect === "choose-pay-atm3") {
        options = ["Visa", "Mastercard", "American Express", "JCB", "Discover"];
    } else if (firstSelect === "choose-pay-cash3") {
        options = ["Không có"];
    }
    ...
    //   <!-- thông tin trong table  -->
        function saveInfo3() {
                var name = document.getElementById("custom3").value;
                var phone = document.getElementById("phone3").value;
                var address = document.getElementById("address3").value;
                var car = document.getElementById("car3").value;
                var paymentMethod = document.getElementById("choose-pay3").value;
                var bank = document.getElementById("select23").value;
                var price = document.getElementById("money3").value;

                if (!name || !phone || !address || !car || !paymentMethod || !bank) {
                    alertWarning3("Vui lòng điền đầy đủ thông tin!");
                } else {
                    if (paymentMethod === "choose-pay-cash3") {
                       paymentMethod = "Tiền mặt";
                    } else if (paymentMethod === "choose-pay-mobile-banking3") {
                       paymentMethod = "Chuyển khoản qua Ngân Hàng";
                    } else if (paymentMethod === "choose-pay-atm3") {
                ...
                if(typeof(Storage) !== "undefined")
                    {
                        try {
                            ...
                            // Lưu mảng đã cập nhật vào localStorage
                             ...
                             alertSuccess3("Đặt hàng thành công");
       ...
       function alertSuccess3(message) {
         var alertBox = $('<div class="alert alert-success alert-dismissible fade show" role="alert">' + 
                         ...
                         '</div>');
         $('#alerts3').append(alertBox);
         setTimeout(function() {
          alertBox.alert('close');
          }, 1000); 
       }

       function alertWarning3(message) {
