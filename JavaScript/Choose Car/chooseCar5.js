...
function showSecondSelect4() {
    var firstSelect = document.getElementById("choose-pay4").value;
    var secondSelectContainer = document.getElementById("select2Container4");
    var secondSelect = document.getElementById("select24");
    ...
    if (firstSelect === "choose-pay-mobile-banking4") {
        options = ["Vietcombank", "Techcombank", "VPBank", "BIDV", "MBBank", "Agribank"];
    } else if (firstSelect === "choose-pay-atm4") {
        options = ["Visa", "Mastercard", "American Express", "JCB", "Discover"];
    } else if (firstSelect === "choose-pay-cash4") {
        options = ["Không có"];
    }
    ...
    //   <!-- thông tin trong table  -->
        function saveInfo4() {
                var name = document.getElementById("custom4").value;
                var phone = document.getElementById("phone4").value;
                var address = document.getElementById("address4").value;
                var car = document.getElementById("car4").value;
                var paymentMethod = document.getElementById("choose-pay4").value;
                var bank = document.getElementById("select24").value;
                var price = document.getElementById("money4").value;

                if (!name || !phone || !address || !car || !paymentMethod || !bank) {
                    alertWarning4("Vui lòng điền đầy đủ thông tin!");
                } else {
                    if (paymentMethod === "choose-pay-cash4") {
                       paymentMethod = "Tiền mặt";
                    } else if (paymentMethod === "choose-pay-mobile-banking4") {
                       paymentMethod = "Chuyển khoản qua Ngân Hàng";
                    } else if (paymentMethod === "choose-pay-atm4") {
                ...
                if(typeof(Storage) !== "undefined")
                    {
                        try {
                            ...
                            // Lưu mảng đã cập nhật vào localStorage
                             ...
                             alertSuccess4("Đặt hàng thành công");
