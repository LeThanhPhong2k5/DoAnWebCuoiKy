...
function showSecondSelect6() {
    var firstSelect = document.getElementById("choose-pay6").value;
    var secondSelectContainer = document.getElementById("select2Container6");
    var secondSelect = document.getElementById("select26");
    ...
    if (firstSelect === "choose-pay-mobile-banking6") {
        options = ["Vietcombank", "Techcombank", "VPBank", "BIDV", "MBBank", "Agribank"];
    } else if (firstSelect === "choose-pay-atm6") {
        options = ["Visa", "Mastercard", "American Express", "JCB", "Discover"];
    } else if (firstSelect === "choose-pay-cash6") {
        options = ["Không có"];
    }
    ...
    //   <!-- thông tin trong table  -->
        function saveInfo6() {
                var name = document.getElementById("custom6").value;
                var phone = document.getElementById("phone6").value;
                var address = document.getElementById("address6").value;
                var car = document.getElementById("car6").value;
                var paymentMethod = document.getElementById("choose-pay6").value;
                var bank = document.getElementById("select26").value;
                var price = document.getElementById("money6").value;

                if (!name || !phone || !address || !car || !paymentMethod || !bank) {
                    alertWarning5("Vui lòng điền đầy đủ thông tin!");
                } else {
                    if (paymentMethod === "choose-pay-cash6") {
                       paymentMethod = "Tiền mặt";
                    } else if (paymentMethod === "choose-pay-mobile-banking6") {
                       paymentMethod = "Chuyển khoản qua Ngân Hàng";
                    } else if (paymentMethod === "choose-pay-atm6") {
                ...
                if(typeof(Storage) !== "undefined")
                    {
                        try {
                            ...
                            // Lưu mảng đã cập nhật vào localStorage
                             ...
                             alertSuccess6("Đặt hàng thành công");
