...
function showSecondSelect5() {
    var firstSelect = document.getElementById("choose-pay5").value;
    var secondSelectContainer = document.getElementById("select2Container5");
    var secondSelect = document.getElementById("select25");
    ...
    if (firstSelect === "choose-pay-mobile-banking5") {
        options = ["Vietcombank", "Techcombank", "VPBank", "BIDV", "MBBank", "Agribank"];
    } else if (firstSelect === "choose-pay-atm5") {
        options = ["Visa", "Mastercard", "American Express", "JCB", "Discover"];
    } else if (firstSelect === "choose-pay-cash5") {
        options = ["Không có"];
    }
    ...
    //   <!-- thông tin trong table  -->
        function saveInfo5() {
                var name = document.getElementById("custom5").value;
                var phone = document.getElementById("phone5").value;
                var address = document.getElementById("address5").value;
                var car = document.getElementById("car5").value;
                var paymentMethod = document.getElementById("choose-pay5").value;
                var bank = document.getElementById("select25").value;
                var price = document.getElementById("money5").value;

                if (!name || !phone || !address || !car || !paymentMethod || !bank) {
                    alertWarning5("Vui lòng điền đầy đủ thông tin!");
                } else {
                    if (paymentMethod === "choose-pay-cash5") {
                       paymentMethod = "Tiền mặt";
                    } else if (paymentMethod === "choose-pay-mobile-banking5") {
                       paymentMethod = "Chuyển khoản qua Ngân Hàng";
                    } else if (paymentMethod === "choose-pay-atm5") {
                ...
                if(typeof(Storage) !== "undefined")
                    {
                        try {
                            ...
                            // Lưu mảng đã cập nhật vào localStorage
                             ...
                             alertSuccess5("Đặt hàng thành công");
