...
function showSecondSelect1() {
    var firstSelect = document.getElementById("choose-pay1").value;
    var secondSelectContainer = document.getElementById("select2Container1");
    var secondSelect = document.getElementById("select21");
    ...
    if (firstSelect === "choose-pay-mobile-banking1") {
        options = ["Vietcombank", "Techcombank", "VPBank", "BIDV", "MBBank", "Agribank"];
    } else if (firstSelect === "choose-pay-atm1") {
        options = ["Visa", "Mastercard", "American Express", "JCB", "Discover"];
    } else if (firstSelect === "choose-pay-cash1") {
        options = ["Không có"];
    }
    ...
    //   <!-- thông tin trong table  -->
        function saveInfo1() {
                var name = document.getElementById("custom1").value;
                var phone = document.getElementById("phone1").value;
                var address = document.getElementById("address1").value;
                var car = document.getElementById("car1").value;
                var paymentMethod = document.getElementById("choose-pay1").value;
                var bank = document.getElementById("select21").value;
                var price = document.getElementById("money1").value;

                if (!name || !phone || !address || !car || !paymentMethod || !bank) {
                    alertWarning1("Vui lòng điền đầy đủ thông tin!");
                } else {
                    if (paymentMethod === "choose-pay-cash1") {
                       paymentMethod = "Tiền mặt";
                    } else if (paymentMethod === "choose-pay-mobile-banking1") {
                       paymentMethod = "Chuyển khoản qua Ngân Hàng";
                    } else if (paymentMethod === "choose-pay-atm1") {
                ...
                if(typeof(Storage) !== "undefined")
                    {
                        try {
                            ...
                            // Lưu mảng đã cập nhật vào localStorage
                             ...
                             alertSuccess1("Đặt hàng thành công");
