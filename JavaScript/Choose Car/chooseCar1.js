// <!-- Js lựa chọn kiểu chuyển khoản -->
    ...
    if (firstSelect === "choose-pay-mobile-banking") {
    ...
    } else if (firstSelect === "choose-pay-cash") {
        options = ["Không có"];
    }
    ...
    //   <!-- thông tin trong table  -->
        function saveInfo() {
                ...
                var paymentMethod = document.getElementById("choose-pay").value;
                var bank = document.getElementById("select2").value;
                var price = document.getElementById("money").value;

                if (!name || !phone || !address || !car || !paymentMethod || !bank) {
                    alertWarning("Vui lòng điền đầy đủ thông tin!");
                } else {
                    if (paymentMethod === "choose-pay-cash") {
                       paymentMethod = "Tiền mặt";
                    } else if (paymentMethod === "choose-pay-mobile-banking") {
                       paymentMethod = "Chuyển khoản qua Ngân Hàng";
                    } else if (paymentMethod === "choose-pay-atm") {
